...
use sha512_halo2::sha512::{BlockWord, Sha512 as OtherSha512, Table16Chip, Table16Config};
...
fn sha512_test() {
    ...
    impl Circuit<bn256::Fr> for MyCircuit {
        ...
        fn synthesize(
            ...
            println!("CIRCUIT DIGEST={:?}", digest.0);
            ...
            for (idx, digest_word) in digest.0.iter().enumerate() {
                digest_word.0.assert_if_known(|digest| *digest == s[idx])
            }
