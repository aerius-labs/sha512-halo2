...
pub const COMPRESSION_OUTPUT: [u64; 8] = [
    ...
    0b1100110001000001011100110100100110101110001000000100000100110001,
    0b0001001011100110111110100100111010001001101010010111111010100010,
    0b0000101010011110111011101110011001001011010101011101001110011010,
    0b0010000110010010100110010010101000100111010011111100000110101000,
    0b0011011010111010001111000010001110100011111111101110101110111101,
    0b0100010101001101010001000010001101100100001111001110100000001110,
    0b0010101010011010110010010100111110100101010011001010010010011111,
];
...
impl CompressionConfig {
    ...
    pub fn assign_word_halves(
        ...
        let w_lo_lo =
            SpreadVar::without_lookup(region, a_7, row, a_8, row, lo_lo.map(SpreadWord::new))?;
        let w_lo_hi = SpreadVar::without_lookup(
            region,
            a_7,
            row + 1,
            a_8,
            row + 1,
            lo_hi.map(SpreadWord::new),
        )?;
        let w_hi_lo = SpreadVar::without_lookup(
            region,
            a_7,
            row + 2,
            a_8,
            row + 2,
            hi_lo.map(SpreadWord::new),
        )?;
        let w_hi_hi = SpreadVar::without_lookup(
            region,
            a_7,
            row + 3,
            a_8,
            row + 3,
            hi_hi.map(SpreadWord::new),
        )?;
        ...
        let w_hi_d =
            AssignedBits::<32>::assign_bits(region, || "w_hi_d", a_7, row + 1, w_hi_dense)?;

        let w_hi_spread = Self::joinspread(&w_hi_lo, &w_hi_hi);
        let w_hi_s =
            AssignedBits::<64>::assign_bits(region, || "w_hi_s", a_8, row + 1, w_hi_spread)?;

        Ok(((w_lo_d, w_hi_d).into(), (w_lo_s, w_hi_s).into()))
    }
}
